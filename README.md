LookUp
  ...
  const challanresult = await challandata.aggregate([
                {
                    $match: {
                ...
                { $project: { _id: 0, __v: 0 } },
                {
                    $lookup: {
                        from: 'lotdatas',
                        localField: 'challan_number',
                        foreignField: 'challan_number',
                        "pipeline": [
                            {
                                $lookup: {
                                    from: 'userdatas',
                                    localField: 'factory_username',
                ...
                { $project: { 'lot_data._id': 0, 'lot_data.__v': 0 } },
                {
                    $lookup: {
                        from: 'lottransactiondatas',
                        localField: 'challan_number',
                        foreignField: 'challan_number',
                        "pipeline": [
                            { "$project": { _id: 0, __v: 0 } }
                        ],
                        as: "lot_transaction_data"
                    }
                },
                {
                    $lookup: {
                        from: 'designdatas',
                        localField: 'design_no',
                        foreignField: 'design_no',
                        "pipeline": [
                            { "$project": { _id: 0, __v: 0, 'machine_a._id': 0, 'machine_b._id': 0, 'machine_c._id': 0 } }
                        ],
                        as: "design_data"
                    }
                },
                ...
                {
                    $lookup: {
                        from: 'partyreturndatas',
                        localField: 'challan_number',
                        foreignField: 'challan_number',
                        "pipeline": [
                            { "$project": { _id: 0, __v: 0 } }
                        ],
                        as: "party_return_data"
                    }
                },
                {
                    $lookup: {
